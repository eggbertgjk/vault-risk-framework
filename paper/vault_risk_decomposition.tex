...
\textbf{Systemic Risk in Crypto.} Farzulla and Maksakov \cite{farzulla2026asri} propose the Adaptive Systemic Risk Index (ASRI) for crypto markets, using HMM regime detection across four sub-indices (stablecoin concentration, DeFi liquidity, contagion, regulatory opacity). Their framework operates at the macro level---detecting \textit{when} the system is stressed---while ours operates at the micro level, identifying \textit{where} risk concentrates within individual vault architectures. The two approaches are complementary: ASRI signals could trigger portfolio-level rebalancing, while VRS decomposition guides vault-level allocation within each regime. Separately, Farzulla \cite{farzulla2025wp} finds that whitepaper narratives do not predict market factor structure, reinforcing our premise that risk should be measured from observable on-chain properties rather than stated intentions.
...
\begin{tikzpicture}[scale=0.70, every node/.style={font=\scriptsize}]
    ...
    % Row labels (left side) — anchor=east to right-align before bars
    \node[anchor=east, font=\tiny, inner sep=0pt] at (-4.0, 2) {CONTRACT};
    \node[anchor=east, font=\tiny, inner sep=0pt] at (-4.0, 1.2) {ORACLE};
    \node[anchor=east, font=\tiny, inner sep=0pt] at (-4.0, 0.4) {GOVERNANCE};
    \node[anchor=east, font=\tiny, inner sep=0pt] at (-4.0, -0.4) {OPERATIONAL};
...
\bibitem{farzulla2026asri}
M.~Farzulla and A.~Maksakov.
\newblock Adaptive systemic risk index for cryptocurrency markets.
\newblock \textit{arXiv:2602.03874}, 2026.

\bibitem{farzulla2025wp}
M.~Farzulla.
\newblock Do cryptocurrency whitepapers predict market behavior? A factor analysis approach.
\newblock \textit{arXiv:2601.20336}, 2025.
